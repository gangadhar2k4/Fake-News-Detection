    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            context = {
                ...
                'form': NewsVerificationForm()  # Fresh form for another verification
            }
...
def get_trending_topics():
    ...
    if not topics.exists():
        # Create some initial trending topics
        initial_topics = [
...
def update_trending_topics(category, title, content):
    ...
    # Update keyword topics (limit to avoid spam)
    for keyword in keywords[:2]:  # Only top 2 keywords
        if len(keyword) > 4:
